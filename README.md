...
npm run migrate   # or apply SQL files individually — see db/migrations/
npm run dev
```
...
For production on Render + Neon, see [Production deployment](#production-deployment).
...
|---|---|---|
| App | [Render](https://render.com) Web Service (`render.yaml`) | Free tier |
| Database | [Neon](https://neon.tech) Postgres | Free tier |
| LLM | Anthropic API | Pay per review |
| Billing | Stripe (optional until paid plans) | No monthly fee |
...
| CI on push/PR | GitHub Actions runs `npm test` and `npm run build` |

You still configure these manually in external dashboards: **Neon `DATABASE_URL`**,
**Anthropic API key**, optional **Google/ORCID OAuth**, and optional **Stripe**
products + webhook.
...
This repo includes [`render.yaml`](render.yaml) at the root (service name:
`qualisapio` → `https://qualisapio.onrender.com`).

1. Push `main` to GitHub.
2. Render Dashboard → **New** → **Blueprint** → connect `cosmicoral/quallens`.
...
**Manual alternative:** New → Web Service → connect repo, use
`npm ci && npm run build` / `npm start`, and copy env vars from `render.yaml`.
...
|---|---|
| `DATABASE_URL` | Neon connection string — **you provide** |
| `BETTER_AUTH_SECRET` | Auto-generated by Blueprint |
| `BETTER_AUTH_URL` | Pre-set to `https://qualisapio.onrender.com` in Blueprint |
| `NEXT_PUBLIC_APP_URL` | Pre-set to `https://qualisapio.onrender.com` in Blueprint |
| `ANTHROPIC_API_KEY` | Anthropic Console — **you provide** |
| `LLM_PROVIDER` | `anthropic` (default in Blueprint) |
| `LLM_MODEL` | `claude-opus-5` (or a model your key can access) |

#### Email sign-in

No extra variables. Email/password works once the required block above is set.
...
| — | Redirect URI: `https://qualisapio.onrender.com/api/auth/callback/google` |
| — | JavaScript origin: `https://qualisapio.onrender.com` |

#### ORCID OAuth (optional)

| Variable | External setting |
|---|---|
| `ORCID_CLIENT_ID` / `ORCID_CLIENT_SECRET` | [ORCID Developer Tools](https://orcid.org/developer-tools) |
| `ORCID_REDIRECT_URI` | Optional — defaults to `https://qualisapio.onrender.com/api/auth/oauth2/callback/orcid` |
...
```text
https://qualisapio.onrender.com/api/stripe/webhook
```
...
Checkout return alone does **not** upgrade a plan; access changes only after
...
```bash
# Auth health
curl https://qualisapio.onrender.com/api/auth/ok
# → {"ok":true}
```

Then in the browser:

1. Sign up with email → land on `/dashboard`.
2. Open `/settings` — profile saves.
3. Run one review on `/review` (may take several minutes).
4. Confirm the run appears on the dashboard and opens from history.
5. If Stripe is configured: checkout on `/pricing` with test card `4242 4242 4242 4242`,
   confirm webhooks return `200`, and the dashboard shows Plus/Pro.

### 5. Free-tier limitations

- **Render Free** spins down after ~15 minutes idle; the first request after sleep
  can take 30–60 seconds.
- **Neon Free** has storage and connection limits; sufficient for MVP traffic.
- **Anthropic** usage is never free; set spending limits in the Anthropic Console.
- Moving Stripe to **live mode** requires replacing all Stripe keys, Price IDs,
  and the webhook signing secret together.

### 6. Custom domains

In Render → your service → **Custom Domains**, add your domain and follow DNS
instructions. Then update `BETTER_AUTH_URL`, `NEXT_PUBLIC_APP_URL` in
`render.yaml` (or Render env vars), OAuth redirect URIs, and the Stripe webhook
URL to the new domain and redeploy.
...
For production, use the same variables on Render (see
